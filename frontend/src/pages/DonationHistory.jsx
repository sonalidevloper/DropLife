...
const DonationHistory = () => {
  ...
  const [stats, setStats] = useState({ total: 0, thisYear: 0, lifeSaved: 0, nextEligible: null });
  ...
  const filtered = filter === 'all' ? history : history.filter(d => (d.year || new Date(d.createdAt).getFullYear()).toString() === filter);
  const years = [...new Set(history.map(d => new Date(d.date || d.createdAt).getFullYear()))];

  return (
    <Container className="py-4">
      <h3 className="fw-bold mb-4 d-flex align-items-center gap-2">
        <FaHistory className="text-danger" /> Donation History
      </h3>
      ...
      <Row className="g-3 mb-4">
        {[
          ...
          { label: 'Next Eligible', value: stats.nextEligible ? new Date(stats.nextEligible).toLocaleDateString() : 'Now!', color: 'info' },
        ].map(s => (
      ...
      <Card className="shadow-sm border-0">
        <Card.Header className="bg-white d-flex align-items-center justify-content-between">
          <span className="fw-bold d-flex align-items-center gap-2"><FaTint className="text-danger" /> Donation Records</span>
          <Form.Select size="sm" style={{ width: '150px' }} value={filter} onChange={e => setFilter(e.target.value)}>
        ...
        <Card.Body className="p-0">
          {loading ? (
            ...
            <div className="text-center py-5 text-muted">
              <FaTint className="fs-1 text-muted mb-3" />
            ...
            <Table responsive hover className="mb-0">
              ...
              <tbody>
                {filtered.map((d, i) => (
                  <tr key={d._id || i}>
                    <td>{i + 1}</td>
                    <td>{new Date(d.date || d.createdAt).toLocaleDateString()}</td>
                    <td><Badge bg={BLOOD_GROUP_COLORS[d.bloodGroup] || 'secondary'}>{d.bloodGroup || '—'}</Badge></td>
                    <td>{d.units || 1}</td>
                    <td>{d.location || d.camp || 'Blood Bank'}</td>
